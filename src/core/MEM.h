...
/* Pseudocode for MEM-based PSD Estimation */
#include <math.h>
...
void PreprocessPPI(MEM_Context* ctx, float new_rr) {
  // Add new sample to circular buffer
  ctx->buffer[ctx->index] = new_rr;
...
void ProcessNewPPI(MEM_Context* ctx, float new_rr) {
  PreprocessPPI(ctx, new_rr);
...
void printMEMParameters() {
  Serial.printf("LF: %f, HF: %f, LF/HF Ratio: %f\n", HRV_LF, HRV_HF, HRV_LF_HF_Ratio);
}
